...
- **`Scraper`** ([session.py](src/scraper/session.py)) — the public entry point.
  ...
  `get_file` (streamed, abortable), `submit_form`, `ping`. `rotate_proxy()`
  sends Tor NEWNYM (for `TorProxyUrl`) or advances the round-robin index (for
  regular proxies) and resets the transport connection pool. `close()` aborts
  all in-progress requests before releasing transport resources — there is no
  separate public `abort()` on `Scraper`. It holds an `Engine` (as
  `scraper.engine`) and delegates; it is **not** an `httpx.Client` subclass
...
- **`engine/`** — the engine. `Engine` ([core.py](src/scraper/engine/core.py)) runs a
  private asyncio event loop in a background thread. `request()` builds a
  `RequestState` and threads it through an ordered **middleware** chain
  (`build_chain`), whose innermost layer calls the **transport** (`HttpxTransport`
  or `CurlCffiTransport`, chosen by `build_transport`). Each middleware owns one
  concern (throttle, TLS rotation, concurrency, 403/429 retry, challenge solving,
  stealth, hooks, proxy, SSL retry). It is a documented extension surface, but the
  curated public API stays in `__init__`/`config`/`exceptions`.

### Cloudflare-bypass surface

The realistic ceiling of the httpx transport is its TLS (JA3/JA4) and HTTP/2
fingerprint — `set_ciphers()` in [tls.py](src/scraper/engine/tls.py) only reorders
ciphers, so its ClientHello still reads as Python/httpcore. Three features push past that:
...
- **curl_cffi transport** ([engine/transport/curl.py](src/scraper/engine/transport/curl.py)):
  ...
  adapts the result back into an `httpx.Response` (`adapt_curl_response`). The
  ...
  unavailable the engine falls back to `HttpxTransport`. curl_cffi is a **core
  dependency**.
...
- **Impersonation is on by default.** `default_config()` sets
  ...
  (httpx transport). The UA browser family is aligned to the impersonation target
  in `Engine.__init__`.
